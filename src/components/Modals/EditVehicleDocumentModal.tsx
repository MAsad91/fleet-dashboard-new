...
import { useUpdateVehicleDocumentMutation, useListVehiclesQuery, useGetVehicleDocumentByIdQuery } from "@/store/api/fleetApi";
...
export function EditVehicleDocumentModal({ isOpen, onClose, onSuccess, documentId }: EditVehicleDocumentModalProps) {
  const [updateDocument, { isLoading }] = useUpdateVehicleDocumentMutation();
  const { data: vehiclesData, isLoading: vehiclesLoading } = useListVehiclesQuery({});
  const { data: documentData, isLoading: documentLoading } = useGetVehicleDocumentByIdQuery(
    documentId?.toString() || "",
    { skip: !documentId }
  );
  ...
  useEffect(() => {
    if (documentData) {
      setFormData({
        vehicle: documentData.vehicle?.toString() || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateDocument({ id: documentId.toString(), body: formDataToSend }).unwrap();
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-1 text-center">
              ...
              {documentData?.document_file && (
                <p className="text-sm text-blue-600">
                  Current file: {documentData.document_file.split('/').pop()}
                </p>
              )}
              {formData.document_file && (
                <p className="text-sm text-green-600">
                  New file: {formData.document_file.name}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
          ...
          />
          <Button
            label={isLoading ? "Updating..." : "Update Document"}
            variant="primary"
